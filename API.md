...
`GET /api/voices` 使用查询参数 `speak_model_id`，不使用 `modelId`。

当前只支持两个 CustomVoice backbone：
...
列出指定 `speak_model_id` 对应 backbone 下可用的音色：
...
```text
GET /api/voices?speak_model_id=Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice
```

说明：

- `speak_model_id` 是请求必填字段
- 当前只支持：
  - `Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice`
  - `Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice`
